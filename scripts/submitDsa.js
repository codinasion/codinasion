...
export default async function submitDsa(
  OWNER,
  REPO,
  TOKEN,
  USERNAME,
  ...
  const description = `Write a program for ${ISSUE_TITLE.toLowerCase()}
...
`;
  ...
  if (!fs.existsSync(dsaFolderPath)) {
    await fs.mkdirSync(dsaFolderPath);
  }
  const dsaFilePath = `${dsaFolderPath}/dsa.json`;
  // create file if not exists
  if (!fs.existsSync(dsaFilePath)) {
    await fs.writeFileSync(dsaFilePath, JSON.stringify([]));
  }
  const prevData = await fs.readFileSync(dsaFilePath);
...
It seems that this dsa program already been submitted.

Please check the list of dsa programs here: https://github.com/${OWNER}/${REPO}/blob/master/data/dsa.json
...
`,
  ...
  await fs.writeFileSync(
    ...
    (err) => {
      ...
      console.log("dsa program data written to file");
    }
  );
